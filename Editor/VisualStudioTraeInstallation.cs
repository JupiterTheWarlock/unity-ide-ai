...
";
		...
		private Process FindRunningTraeWithSolution(string solutionPath)
		{
			...
			foreach (var process in processes)
			{
				try
				{
					var workspaces = GetTraeProcessWorkspaces(process);
		...
		private static string[] GetTraeProcessWorkspaces(Process process)
		{
			if (process == null)
				return null;
...
#if UNITY_EDITOR_OSX
				traeStoragePath = IOPath.Combine(userProfile, "Library", "Application Support", "Trae", "User", "workspaceStorage");
#elif UNITY_EDITOR_LINUX
				traeStoragePath = IOPath.Combine(userProfile, ".config", "Trae", "User", "workspaceStorage");
#else
				traeStoragePath = IOPath.Combine(userProfile, "AppData", "Roaming", "Trae", "User", "workspaceStorage");
#endif
