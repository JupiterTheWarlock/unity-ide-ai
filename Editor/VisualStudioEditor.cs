...
namespace Microsoft.Unity.VisualStudio.Editor
{
	...
	public class VisualStudioEditor : IExternalCodeEditor
	{
		...
		public void OnGUI()
		{
			...
			if (installation is VisualStudioQoderInstallation)
			{
				var reuseWindow = EditorPrefs.GetBool(VisualStudioQoderInstallation.ReuseExistingWindowKey, false);
				var newReuseWindow = EditorGUILayout.Toggle(new GUIContent("Reuse existing Qoder window", "When enabled, opens files in an existing Qoder window if found. When disabled, always opens a new window."), reuseWindow);
				if (newReuseWindow != reuseWindow)
					EditorPrefs.SetBool(VisualStudioQoderInstallation.ReuseExistingWindowKey, newReuseWindow);
