...
";
		...
		private Process FindRunningQoderWithSolution(string solutionPath)
		{
			...
			foreach (var process in processes)
			{
				try
				{
					var workspaces = GetQoderProcessWorkspaces(process);
				...
				{
					Debug.LogError($"[Qoder] Error checking process: {ex}");
					continue;
				}
			}
			return null;
		...
		private static string[] GetQoderProcessWorkspaces(Process process)
		{
			if (process == null)
				return null;
			...
			try
			{
				...
				string qoderStoragePath;

#if UNITY_EDITOR_OSX
				qoderStoragePath = IOPath.Combine(userProfile, "Library", "Application Support", "Qoder", "User", "workspaceStorage");
#elif UNITY_EDITOR_LINUX
				qoderStoragePath = IOPath.Combine(userProfile, ".config", "Qoder", "User", "workspaceStorage");
#else
				qoderStoragePath = IOPath.Combine(userProfile, "AppData", "Roaming", "Qoder", "User", "workspaceStorage");
#endif

				if (Directory.Exists(qoderStoragePath))
				{
					foreach (var workspaceDir in Directory.GetDirectories(qoderStoragePath))
					{
						try
						{
							var workspaceStatePath = IOPath.Combine(workspaceDir, "workspace.json");
							...
							var windowStatePath = IOPath.Combine(workspaceDir, "window.json");
						...
						{
							Debug.LogWarning($"[Qoder] Error reading workspace state file: {ex.Message}");
							continue;
						}
					}
				}
			...
			{
				Debug.LogError($"[Qoder] Error getting workspace directory: {ex.Message}");
				return null;
			}
		}
		...
		public override bool Open(string path, int line, int column, string solution)
		{
			...
			if (EditorPrefs.GetBool(ReuseExistingWindowKey, false))
			{
				...
				if (existingProcess != null)
				{
					...
					{
						Debug.LogError($"[Qoder] Error using existing instance: {ex}");
					}
				}
			}
